        ...
        Citizen.Wait(0)
            ...
            for i = 1, #shisha do
                ...
                if dist < 3.5 then
					...
					if not buyad then
						ESX.Game.Utils.DrawText3D(vector3(shishaPos.x, shishaPos.y, shishaPos.z+1), "[~y~E~w~] Inciar session. (10€)", 0.7)
					end
						...
						Citizen.CreateThread(function()
							while true do
								if sessionStarted == true then
									ESX.Game.Utils.DrawText3D(vector3(shishaPos.x, shishaPos.y, shishaPos.z+0.4), "~b~\"H\"~c~ for smoking.", 0.7)
									ESX.Game.Utils.DrawText3D(loc, "~b~\"F\"~c~ to finish session.", 0.7)
								end
								Citizen.Wait(0)
							end
						end)
